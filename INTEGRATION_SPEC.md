...
> **用语**：本文档用 **vDisk 控制中心** 指课表数据的接收与管理端；实现与客户端配置中仍常见 **PlusAPI**、URL 路径中的 `plusapi` 等历史命名，含义与本文「控制中心」侧一致。  
...
**vDisk 控制中心**（教室智能管理 / 集控侧）使用课表数据用于：
...
Token 在 vDisk 控制中心 管理后台 → API Token 管理 中创建和查询。
...
第二步：字段转换
  ...
  - classroom   ← 来源的教室名称（注意大小写需与 vDisk 控制中心 中教室名一致）
...
第四步：验证
  - 检查响应 success=true
  - 登录 vDisk 控制中心 管理后台 → 课表管理 中查看导入结果
```
...
1. **教室名称匹配**：`classroom` 字段值须与 vDisk 控制中心 中已有教室名称**完全一致**（大小写、全半角），否则系统会创建一条新教室记录，可能导致与已有教室重复。
